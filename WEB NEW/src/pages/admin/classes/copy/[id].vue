<template>
    <v-row>
      <v-col cols="12" class="d-flex align-center ga-2 mb-4">
        ...
        <div>
          <h4 class="text-h4 font-weight-bold">Copy Class</h4>
          <p class="text-body-small text-medium-emphasis mb-0">
            Copy an existing class with a new schedule.
          </p>
        </div>
      </v-col>
    </v-row>
...
const update = async () => {
  ...
  try {
    const { id, ...body } = formInput.value as Record<string, unknown> & { id?: unknown };
    const { data } = await axios.post(`/classes`, body);
